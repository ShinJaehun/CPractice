...
void insert_data(struct Data * data, int num_datas);
void print_file_datas(int num_datas);
...
void print_datas(struct Data *datas, int num_datas) {
	...
	while(1){
		...
		if (sel < 1) sel = 1;
		if (sel > num_datas) sel = num_datas;
...
void insert_data(struct Data * datas, int num_datas) {
	
	char *nda = (char *)malloc(sizeof(char) * 9); //날짜
    char *nti = (char *)malloc(sizeof(char) * 5); //시간
    int npr; //중요도
    int ngr; //그룹
    char *nna = (char *)malloc(sizeof(char) * 127); //제목
    char *nde = (char *)malloc(sizeof(char) * 127); //설명
	...
	num_datas++;
	printf("num_datas : %d\n", num_datas);
	datas = (struct Data *)realloc(datas, num_datas);
	
	datas[num_datas].date = (char *)malloc(sizeof(char) * 9); //날짜
    strcpy(datas[num_datas].date, nda);

    datas[num_datas].time = (char *)malloc(sizeof(char) * 5); //시간
    strcpy(datas[num_datas].time, nti);

    datas[num_datas].priority = npr; //중요도
    datas[num_datas].group = ngr; //그룹

    datas[num_datas].name = (char *)malloc(sizeof(char) * (strlen(nna) + 1)); //제목
    strcpy(datas[num_datas].name, nna);

    datas[num_datas].description = (char *)malloc(sizeof(char) * (strlen(nde) + 1)); //시간
    strcpy(datas[num_datas].description, nde);
	    
    printf("%d : %s\n", num_datas, datas[num_datas].date);
    printf("%d : %s\n", num_datas, datas[num_datas].time);
    printf("%d : %d\n", num_datas, datas[num_datas].group);
    printf("%d : %d\n", num_datas, datas[num_datas].priority);
    printf("%d : %s\n", num_datas, datas[num_datas].name);
    printf("%d : %s\n", num_datas, datas[num_datas].description);
	    
	FILE *file = fopen("C:\\project\\c\\Schedule\\data.txt", "a"); //자료를 받아오기 위해 파일 다시 열기
	fprintf(file, "\n%s %s %d %d %s %s", nda, nti, npr, ngr, nna, nde);
	fclose(file);
